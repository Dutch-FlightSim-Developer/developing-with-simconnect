...
auto main([[maybe_unused]] int argc, [[maybe_unused]] const char* argv[]) -> int // NOLINT(bugprone-exception-escape)
{
    ...
    facilityHandler.logger().level(logLevel);

    auto request = facilityHandler.requestJetwayData(airport, {},
        [](const Facilities::Jetway& jetway) {
                std::cout << std::format("Received jetway data for {} parking spot {}:\n", jetway.icaoCode.data(), jetway.parkingIndex);
                std::cout << std::format("- Position: lat {}, lon {}, alt {}\n", jetway.position.Latitude, jetway.position.Longitude, jetway.position.Altitude);
                std::cout << std::format("- Orientation: pitch {}, bank {}, heading {}\n", jetway.orientation.Pitch, jetway.orientation.Bank, jetway.orientation.Heading);
                std::cout << std::format("- State: {}\n", static_cast<int>(jetway.state));
                std::cout << std::format("- Door index: {}\n", jetway.doorIndex);
                std::cout << std::format("- Exit door position: x {}, y {}, z {}\n", jetway.exitDoorPosition.x, jetway.exitDoorPosition.y, jetway.exitDoorPosition.z);
                std::cout << std::format("- Main handle position: x {}, y {}, z {}\n", jetway.mainHandlePosition.x, jetway.mainHandlePosition.y, jetway.mainHandlePosition.z);
                std::cout << std::format("- Secondary handle position: x {}, y {}, z {}\n", jetway.secondaryHandlePosition.x, jetway.secondaryHandlePosition.y, jetway.secondaryHandlePosition.z);
                std::cout << std::format("- Wheel ground lock position: x {}, y {}, z {}\n", jetway.wheelGroundLockPosition.x, jetway.wheelGroundLockPosition.y, jetway.wheelGroundLockPosition.z);
        });

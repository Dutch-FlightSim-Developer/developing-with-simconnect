...
constexpr static const char* appName = "ClientData Sender";
...
static bool setupClientData()
{
    ...
    std::cerr << "[Client data area created and definition registered.]\n";
    return true;
}
...
static bool sendMessage(const char* text)
{
    ...
    std::cerr << std::format("[Sent: '{}']\n", text);
    return true;
}


/**
 * Run the message loop, sending a new message every @p interval until @p numUpdates
 * messages have been sent or the simulator shuts down.
 *
 * @param numUpdates Number of messages to send before exiting.
 * @param interval   Time to wait between messages.
 */
static void handleMessages(int numUpdates, std::chrono::seconds interval)
{
    std::cerr << std::format("[Sending {} updates, one every {} second(s).]\n",
        numUpdates, interval.count());

    int updateCount = 0;
    auto nextSendTime = std::chrono::steady_clock::now(); // send immediately on first tick
    ...
    while (updateCount < numUpdates) {
        ...
        // Send the next update when the interval has elapsed
        const auto now = std::chrono::steady_clock::now();
        if (now >= nextSendTime) {
            ++updateCount;
            sendMessage(std::format("Hello, World! (update {})", updateCount).c_str());
            nextSendTime = now + interval;
        }
    }

    std::cerr << "[All updates sent.]\n";
}

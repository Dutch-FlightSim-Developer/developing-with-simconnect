...
constexpr static const char* appName = "PingPong Pong";
 ...
 *
 * pong does not create the area — ping owns it. pong only maps the same name
 * to the same ID, defines the same layout, and subscribes ON_SET.
 *
...
/**
 * Run the message loop until the simulator shuts down, responding to Ping messages
 * by scheduling a Pong after a brief delay.
 */
        ...
        while (SUCCEEDED(SimConnect_GetNextDispatch(hSimConnect, &pData, &cbData))) {
            ...
            case SIMCONNECT_RECV_ID_CLIENT_DATA:
            {
                ...
                if (pClientData->dwRequestID == REQ_ID) {
                    ...
                    if (std::strcmp(data->message, "Ping") == 0) {
                        std::cout << "Ping!\n";

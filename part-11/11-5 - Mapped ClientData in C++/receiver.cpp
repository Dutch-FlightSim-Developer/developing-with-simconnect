...
// Mapped ClientData — Receiver (C++ version of 11-1 receiver)
//
// Maps the same client data area name and subscribes using the same
// MappedClientDataDefinition.  On each delivery the callback receives a typed
// HelloWorldData reference — no raw pointer arithmetic, no manual cast.
//
...
void runTest()
{
  ...
  if (connection.open()) {
    // The receiver does NOT call createClientData — the sender owns the area.
    const ClientDataId dataId = dataHandler.mapClientDataName(CLIENT_DATA_NAME);

    // Same field layout as the sender.  The library delivers each update via
    // a direct reinterpret_cast because the fields cover the full struct.
    ...
    auto req = dataHandler.requestClientData(
      dataId, def,
      [](const HelloWorldData& data) {
        std::cout << std::format("[{}] '{}'\n", data.updateCount, data.message.data());
      },

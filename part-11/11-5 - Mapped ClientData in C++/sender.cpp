...
// Mapped ClientData — Sender (C++ version of 11-1 sender)
//
// Creates a read-only client data area and registers its layout using
// MappedClientDataDefinition.  Because all fields together cover exactly
// sizeof(HelloWorldData), useMapping() is true and the library delivers each
// update via a direct reinterpret_cast — no per-field unmarshalling.
//
// Sends 10 updates, one every two seconds, then exits.
// Start the receiver before this program.
...
void runTest()
{
  ...
  if (connection.open()) {
    ...
    dataHandler.createClientData(dataId, sizeof(HelloWorldData));

    // Register field layout.  message[256] + updateCount[4] == sizeof(HelloWorldData),
    // so useMapping() is true and sendClientData() sends the struct as a raw blob.

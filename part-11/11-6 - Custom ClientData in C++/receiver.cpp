...
// Custom ClientData — Receiver
//
// Maps the same area name and subscribes with the same CustomClientDataDefinition.
// The setters convert received int32_t wire values back to decimal strings in
// FlightData; the callback then prints both fields.
//
...
void runTest()
{
  ...
  if (connection.open()) {
    // The receiver does NOT create the area — the sender owns it.
    const ClientDataId dataId = dataHandler.mapClientDataName(CLIENT_DATA_NAME);

    // Same two int32_t datums as the sender.  Only setters needed here —
    // this side never sends, so getter lambdas are empty.
    ...
    auto req = dataHandler.requestClientData(
      ...
      [](const FlightData& data) {
        std::cout << std::format("Altitude: {} ft   Speed: {} kts\n",

...
// Custom ClientData — Sender
//
// Accepts altitude and speed as command-line arguments, stores them as strings
// in FlightData, and sends them as int32_t wire values using a
// CustomClientDataDefinition.  The getters parse the string fields into int32_t
// for the wire; the setters on the receiver side do the reverse.
//
// Usage: sender --altitude=<ft> --speed=<kts>
//   e.g. sender --altitude=35000 --speed=450
...
void runTest(int argc, const char* argv[]) // NOLINT(cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays,modernize-avoid-c-arrays)
{
  const auto args = gatherArgs(argc, argv);
  if (!args.contains("altitude") || !args.contains("speed")) {
    std::cerr << "Usage: sender --altitude=<ft> --speed=<kts>\n";
    return;
  }
  const auto& altStr   = args.at("altitude");
  const auto& speedStr = args.at("speed");
  ...
  if (connection.open()) {
    // Create as read-only: only this sender writes.  Wire size is 2 × int32_t = 8 bytes.
    const ClientDataId dataId = dataHandler.mapClientDataName(CLIENT_DATA_NAME);
    dataHandler.createClientData(dataId, 2 * sizeof(int32_t));

    // Two int32_t datums — struct holds decimal strings, wire holds integers.
    // Getter: parse string → int32_t for the wire.
    // Setter: int32_t from wire → decimal string in struct (unused on send path).
    CustomClientDataDefinition<FlightData> def;
    def.addField(
      ...
      [](Data::DataBlockBuilder& builder, const FlightData& data) {
        builder.addInt32(std::stoi(std::string{data.altitudeStr.data()}));
      });
    def.addField(
      ClientDataType::int32,
      [](FlightData& data, Data::DataBlockReader& reader) {
    ...
    std::cerr << std::format("[Sent: altitude={} ft, speed={} kts]\n",
      data.altitudeStr.data(), data.speedStr.data());
  } else {

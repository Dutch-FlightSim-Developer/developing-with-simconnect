...
// PingPong — Ping (C++ version of 11-2 ping, using StatelessClientDataDefinition)
//
// Creates a shared client data area with a single int32_t datum.  There is no
// backing struct — StatelessClientDataDefinition wires the datum directly to a
// per-field getter (for sending) and setter (for receiving).
//
// ping writes PING (1) first.  Whenever the setter fires with PONG (2), ping
// schedules another PING after one second.
//
// Press Ctrl+C or close the simulator to stop.
...
constexpr auto kPingInterval    = std::chrono::seconds{1};      ///< Delay before re-pinging after a Pong is received.
constexpr auto kDispatchInterval = std::chrono::milliseconds{100}; ///< Polling interval for the dispatch loop.
...
void runTest()
{
  ...
  if (connection.open()) {
    // Create a shared (writable by both sides) area for one int32_t datum.
    const ClientDataId dataId = dataHandler.mapClientDataName(CLIENT_DATA_NAME);
    dataHandler.createSharedClientData(dataId, sizeof(int32_t));

    // No backing struct.  The setter reacts to PONG; the getter always sends PING.
    std::optional<std::chrono::steady_clock::time_point> replyAt;

    StatelessClientDataDefinition def;
    def.addInt32(
      [&replyAt](int32_t value) {
        if (value == PONG) {
          std::cout << "  Pong!\n";
          replyAt = std::chrono::steady_clock::now() + kPingInterval;
        }
      },
      []() { return PING; });

    auto req = dataHandler.requestClientData(
      dataId, def,
    ...
    while (connection.isOpen()) {
      handler.dispatchFor(kDispatchInterval);
      if (replyAt && std::chrono::steady_clock::now() >= *replyAt) {
        replyAt.reset();
        dataHandler.sendClientData(dataId, def);
        std::cout << "Ping!\n";
      }

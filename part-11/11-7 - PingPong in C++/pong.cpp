...
// PingPong — Pong (C++ version of 11-2 pong, using StatelessClientDataDefinition)
//
// Maps the shared client data area (created by ping) and subscribes.
// When the setter fires with PING (1), pong schedules a reply after 500ms.
// The getter always returns PONG (2).
//
// Start pong before ping.  Press Ctrl+C or close the simulator to stop.
...
constexpr auto kPongDelay        = std::chrono::milliseconds{500}; ///< Delay before replying to a Ping.
...
void runTest()
{
  ...
  if (connection.open()) {
    // Pong does NOT create the area — ping owns it.
    const ClientDataId dataId = dataHandler.mapClientDataName(CLIENT_DATA_NAME);

    // Same structure as ping.  Setter reacts to PING; getter always sends PONG.
    ...
    def.addInt32(
      [&replyAt](int32_t value) {
        if (value == PING) {
          std::cout << "Ping!\n";
          replyAt = std::chrono::steady_clock::now() + kPongDelay;
        }
      },
      []() { return PONG; });

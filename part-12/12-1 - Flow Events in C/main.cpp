...
constexpr static const char* appName = "12-1 - Flow Events in C";
...
#if MSFS_2024_SDK
/**
 * Handle a Flow Event message from SimConnect.
 *
 * @param msg The flow event message to handle.
 */
static void handleFlowEvent(const SIMCONNECT_RECV_FLOW_EVENT& msg)
{
    switch (msg.FlowEvent)
    {
    case SIMCONNECT_FLOW_EVENT_NONE:
        std::cout << "Flow event: None\n";
        break;
    case SIMCONNECT_FLOW_EVENT_FLT_LOAD:
        std::cout << "Flow event: Flight file loading\n";
        break;
    case SIMCONNECT_FLOW_EVENT_FLT_LOADED:
        std::cout << "Flow event: Flight file loaded\n";
        break;
    case SIMCONNECT_FLOW_EVENT_TELEPORT_START:
        std::cout << "Flow event: Teleport starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_TELEPORT_DONE:
        std::cout << "Flow event: Teleport complete\n";
        break;
    case SIMCONNECT_FLOW_EVENT_BACK_ON_TRACK_START:
        std::cout << "Flow event: Back on track starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_BACK_ON_TRACK_DONE:
        std::cout << "Flow event: Back on track complete\n";
        break;
    case SIMCONNECT_FLOW_EVENT_SKIP_START:
        std::cout << "Flow event: Skip starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_SKIP_DONE:
        std::cout << "Flow event: Skip complete\n";
        break;
    case SIMCONNECT_FLOW_EVENT_BACK_TO_MAIN_MENU:
        std::cout << "Flow event: Returning to main menu\n";
        break;
    case SIMCONNECT_FLOW_EVENT_RTC_START:
        std::cout << "Flow event: Real-Time Communication starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_RTC_END:
        std::cout << "Flow event: Real-Time Communication ended\n";
        break;
    case SIMCONNECT_FLOW_EVENT_REPLAY_START:
        std::cout << "Flow event: Replay starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_REPLAY_END:
        std::cout << "Flow event: Replay ended\n";
        break;
    case SIMCONNECT_FLOW_EVENT_FLIGHT_START:
        std::cout << "Flow event: Flight starting\n";
        break;
    case SIMCONNECT_FLOW_EVENT_FLIGHT_END:
        std::cout << "Flow event: Flight ended\n";
        break;
    case SIMCONNECT_FLOW_EVENT_PLANE_CRASH:
        std::cout << "Flow event: Plane crashed\n";
        break;
    default:
        std::cout << std::format("Flow event: Unknown ({})\n", static_cast<int>(msg.FlowEvent));
        break;
    }
    if (msg.FltPath[0] != '\0') {
...
/**
 * Handle incoming SimConnect messages for the given duration.
 *
 * @param duration How long to run the message loop. Pass 0s to run until the simulator shuts down.
 */
static void handleMessages(std::chrono::seconds duration)
{
    ...
    while (haveNoDeadline || (std::chrono::steady_clock::now() <= endTime)) {
        ...
        for (HRESULT hr{ S_OK }; (haveNoDeadline || (std::chrono::steady_clock::now() <= endTime)) && SUCCEEDED(hr = SimConnect_GetNextDispatch(hSimConnect, &pData, &cbData)); Sleep(100)) {
            ...
            {
                handleFlowEvent(*toRecvPtr<SIMCONNECT_RECV_FLOW_EVENT>(pData));
            }
            break;
#endif
...
auto main([[maybe_unused]] int argc, [[maybe_unused]] const char* argv[]) -> int
{
...
#if !MSFS_2024_SDK
    std::cerr << "Flow Events require the MSFS 2024 SDK. This example cannot run with the MSFS 2020 SDK.\n";
    return 1;
#else
    if (!connect()) {
        return -1;
    ...
    std::cout << "Subscribing to flow events...\n";
    SimConnect_SubscribeToFlowEvent(hSimConnect);
    std::cout << "Listening for flow events. Trigger them by loading a flight, teleporting, crashing, etc.\n";

    handleMessages(0s);

    SimConnect_UnsubscribeToFlowEvent(hSimConnect);
    disconnect();

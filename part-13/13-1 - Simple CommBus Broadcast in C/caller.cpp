...
constexpr static const char* appName = "CommBus Caller";
static HANDLE hSimConnect{ nullptr };
static HANDLE hEvent{ nullptr };

constexpr static int NUM_BROADCASTS{ 10 };
constexpr static std::chrono::seconds BROADCAST_INTERVAL{ 2 };
...
/**
 * Fire a single CommBus broadcast with the given message text.
 *
 * The event is broadcast to all JS, WASM, and SimConnect subscribers currently
 * listening on COMMBUS_HELLO_EVENT. If no one is subscribed, the call is a no-op —
 * there is no persistent state, unlike ClientData.
 *
 * @param text The message payload.
 * @return true if the call succeeded, false otherwise.
 */
static bool broadcast(const std::string& text)
{
    const DWORD bufferSize = static_cast<DWORD>(text.size() + 1); // include null terminator
    ...
    std::cerr << std::format("[Broadcast: '{}']\n", text);
    return true;
}


/**
 * Run the message loop, firing a CommBus broadcast every BROADCAST_INTERVAL
 * until NUM_BROADCASTS messages have been sent or the simulator shuts down.
 */
static void handleMessages()
{
    std::cerr << std::format("[Sending {} broadcasts, one every {} second(s).]\n",
        NUM_BROADCASTS, BROADCAST_INTERVAL.count());

    int broadcastCount{ 0 };
    auto nextBroadcastTime = std::chrono::steady_clock::now(); // send immediately on first tick

    while (broadcastCount < NUM_BROADCASTS) {
        ...
        if (now >= nextBroadcastTime) {
            ++broadcastCount;
            broadcast(std::format("Hello, World! (broadcast {})", broadcastCount));
            nextBroadcastTime = now + BROADCAST_INTERVAL;
        }
    }

    std::cerr << "[All broadcasts sent.]\n";
}

...
constexpr static const char* appName = "CommBus Listener";
static HANDLE hSimConnect{ nullptr };
static HANDLE hEvent{ nullptr };

/**
 * Local event ID assigned to the COMMBUS_HELLO_EVENT subscription.
 * Used to identify which CommBus event triggered a SIMCONNECT_RECV_ID_COMM_BUS message.
 */
constexpr static SIMCONNECT_CLIENT_EVENT_ID EVENT_HELLO{ 1 };
...
/**
 * Subscribe to COMMBUS_HELLO_EVENT on the CommBus.
 *
 * Unlike ClientData, there is no area to create or data layout to define — just a
 * subscribe call that maps the event name to a local ID.
 *
 * @return true if the subscription succeeded, false otherwise.
 */
static bool subscribe()
{
    const HRESULT hr = SimConnect_SubscribeToCommBusEvent(hSimConnect, EVENT_HELLO, COMMBUS_HELLO_EVENT);
    if (FAILED(hr)) {
        std::cerr << std::format("[Failed to subscribe to CommBus event '{}': 0x{:08X}]\n",
            COMMBUS_HELLO_EVENT, hr);
        return false;
    }
...
/**
 * Unsubscribe from the CommBus event before disconnecting.
 */
static void unsubscribe()
{
    SimConnect_UnsubscribeToCommBusEvent(hSimConnect, EVENT_HELLO);
    std::cerr << "[Unsubscribed from CommBus event.]\n";
}


/**
 * Handle a CommBus message received from the simulator.
 *
 * @param msg The CommBus receive message.
 */
static void handleCommBus(const SIMCONNECT_RECV_COMM_BUS& msg)
{
    if (msg.uEventID == EVENT_HELLO) {
...
/**
 * Run the message loop until the simulator shuts down.
 */
static void handleMessages()
{
    std::cerr << "[Waiting for CommBus broadcasts...]\n";
    ...
    while (true) {
        ...
        while (SUCCEEDED(SimConnect_GetNextDispatch(hSimConnect, &pData, &cbData))) {
            ...
            case SIMCONNECT_RECV_ID_COMM_BUS:
                handleCommBus(*toRecvPtr<SIMCONNECT_RECV_COMM_BUS>(pData));
                break;

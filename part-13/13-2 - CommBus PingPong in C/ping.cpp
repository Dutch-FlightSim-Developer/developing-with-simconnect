...
constexpr static const char* appName = "CommBus Ping";
static HANDLE hSimConnect{ nullptr };
static HANDLE hEvent{ nullptr };

/** Local ID for the Pong subscription — used to identify incoming CommBus messages. */
constexpr static SIMCONNECT_CLIENT_EVENT_ID EVENT_PONG{ 1 };
...
static void handleException(const SIMCONNECT_RECV_EXCEPTION& msg)
{
    ...
    case SIMCONNECT_EXCEPTION_TOO_MANY_MAPS:
        std::cerr << "The maximum number of mappings has been reached. (currently 20)\n";
        break;
    ...
    case SIMCONNECT_EXCEPTION_INVALID_ENUM:
        std::cerr << "The type enum value is unknown.\n";
        break;
    case SIMCONNECT_EXCEPTION_DEFINITION_ERROR:
        std::cerr << "The definition is invalid.\n";
        break;
    case SIMCONNECT_EXCEPTION_DUPLICATE_ID:
        std::cerr << "The ID is already in use.\n";
        break;
    case SIMCONNECT_EXCEPTION_DATUM_ID:
        std::cerr << "Unknown datum ID specified for SetDataOnSimObject.\n";
        break;
    case SIMCONNECT_EXCEPTION_OUT_OF_BOUNDS:
        std::cerr << "The requested value is out of bounds.\n";
        break;
...
/**
 * Subscribe to the Pong event on the CommBus.
 *
 * Each direction of the exchange uses its own event name: Ping calls COMMBUS_PING_EVENT
 * and listens on COMMBUS_PONG_EVENT; Pong does the reverse.
 *
 * @return true if the subscription succeeded, false otherwise.
 */
static bool subscribe()
{
    const HRESULT hr = SimConnect_SubscribeToCommBusEvent(hSimConnect, EVENT_PONG, COMMBUS_PONG_EVENT);
    if (FAILED(hr)) {
        std::cerr << std::format("[Failed to subscribe to CommBus event '{}': 0x{:08X}]\n",
            COMMBUS_PONG_EVENT, hr);
        return false;
    }
    std::cerr << std::format("[Subscribed to CommBus event '{}'.]\n", COMMBUS_PONG_EVENT);
    return true;
}


/**
 * Unsubscribe from the Pong CommBus event.
 */
static void unsubscribe()
{
    SimConnect_UnsubscribeToCommBusEvent(hSimConnect, EVENT_PONG);
    std::cerr << "[Unsubscribed from CommBus event.]\n";
}


/**
 * Fire a Ping on the CommBus.
 *
 * @return true if the call succeeded, false otherwise.
 */
static bool sendPing()
{
    constexpr static const char* payload = "Ping";
    const HRESULT hr = SimConnect_CallCommBusEvent(hSimConnect,
        COMMBUS_PING_EVENT,
        SIMCONNECT_COMM_BUS_BROADCAST_TO_DEFAULT,
        static_cast<DWORD>(std::char_traits<char>::length(payload) + 1),
        payload);

    if (FAILED(hr)) {
        std::cerr << std::format("[Failed to call CommBus event: 0x{:08X}]\n", hr);
        return false;
    }
...
/**
 * Run the Ping side of the exchange.
 *
 * Fires an initial Ping, then waits up to PONG_TIMEOUT for each Pong reply.
 * On receipt, schedules the next Ping after PING_INTERVAL.
 * Exits after MAX_EXCHANGES successful exchanges or on timeout.
 */
static void handleMessages()
{
    std::optional<std::chrono::steady_clock::time_point> timeoutAt{};
    std::optional<std::chrono::steady_clock::time_point> replyAt{};
    int exchangeCount{ 0 };

    auto armTimeout = [&]() {
        timeoutAt = std::chrono::steady_clock::now() + PONG_TIMEOUT;
        replyAt.reset();
    };
    ...
    while (true) {
        ...
        // Determine how long to wait: until the next scheduled event.
        std::chrono::milliseconds waitMs{ 100 };
        if (replyAt) {
            const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(*replyAt - now);
            waitMs = (ms.count() > 0) ? ms : std::chrono::milliseconds{ 0 };
        }
        else if (timeoutAt) {
            const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(*timeoutAt - now);
            waitMs = (ms.count() > 0) ? ms : std::chrono::milliseconds{ 0 };
        }

        ::WaitForSingleObject(hEvent, static_cast<DWORD>(waitMs.count()));
        ...
        while (SUCCEEDED(SimConnect_GetNextDispatch(hSimConnect, &pData, &cbData))) {
            ...
            case SIMCONNECT_RECV_ID_COMM_BUS:
            {
                const auto* pCommBus = toRecvPtr<SIMCONNECT_RECV_COMM_BUS>(pData);
                if (pCommBus->uEventID == EVENT_PONG) {
                    std::cout << "  Pong!\n";
                    ++exchangeCount;
                    timeoutAt.reset();
                    if (exchangeCount >= MAX_EXCHANGES) {
                        std::cerr << std::format("[Completed {} exchanges.]\n", exchangeCount);
                        return;
                    }
                    replyAt = std::chrono::steady_clock::now() + PING_INTERVAL;
                }
            }
            break;
        ...
        const auto nowAfterDispatch = std::chrono::steady_clock::now();

        // Send the next Ping if the inter-exchange delay has elapsed.
        if (replyAt && nowAfterDispatch >= *replyAt) {
            replyAt.reset();
            if (!sendPing()) {
                return;
            }
        ...
        // Give up if no Pong arrived within PONG_TIMEOUT.
        if (timeoutAt && nowAfterDispatch >= *timeoutAt) {
            std::cerr << std::format("[No response within {} seconds. Is Pong running?]\n",
                PONG_TIMEOUT.count());
            return;
        }
    }

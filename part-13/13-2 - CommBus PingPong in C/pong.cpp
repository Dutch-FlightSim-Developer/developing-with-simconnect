...
constexpr static const char* appName = "CommBus Pong";
static HANDLE hSimConnect{ nullptr };
static HANDLE hEvent{ nullptr };

/** Local ID for the Ping subscription — used to identify incoming CommBus messages. */
constexpr static SIMCONNECT_CLIENT_EVENT_ID EVENT_PING{ 1 };
...
/**
 * Subscribe to the Ping event on the CommBus.
 *
 * @return true if the subscription succeeded, false otherwise.
 */
static bool subscribe()
{
    const HRESULT hr = SimConnect_SubscribeToCommBusEvent(hSimConnect, EVENT_PING, COMMBUS_PING_EVENT);
    if (FAILED(hr)) {
        std::cerr << std::format("[Failed to subscribe to CommBus event '{}': 0x{:08X}]\n",
            COMMBUS_PING_EVENT, hr);
        return false;
    }
...
/**
 * Unsubscribe from the Ping CommBus event.
 */
static void unsubscribe()
{
    SimConnect_UnsubscribeToCommBusEvent(hSimConnect, EVENT_PING);
    std::cerr << "[Unsubscribed from CommBus event.]\n";
}


/**
 * Reply with a Pong on the CommBus.
 *
 * @return true if the call succeeded, false otherwise.
 */
static bool sendPong()
{
    constexpr static const char* payload = "Pong";
...
/**
 * Run the Pong side of the exchange.
 *
 * Waits for Ping events and replies immediately with Pong.
 * Runs until the simulator shuts down.
 */
static void handleMessages()
{
    std::cerr << "[Waiting for Ping...]\n";
    ...
    while (true) {
        ...
        while (SUCCEEDED(SimConnect_GetNextDispatch(hSimConnect, &pData, &cbData))) {
            ...
            case SIMCONNECT_RECV_ID_COMM_BUS:
            {
                ...
                if (pCommBus->uEventID == EVENT_PING) {
                    std::cout << "Ping!\n";
                    sendPong();
                }
            }

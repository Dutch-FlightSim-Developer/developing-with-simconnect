...
//
// The companion EFB app (../EFB/) listens for this same event name and displays the
// message. Run this from a terminal while a flight is loaded, e.g.:
//
...
void runTest(int argc, const char* argv[]) // NOLINT(cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays,modernize-avoid-c-arrays)
{
  ...
  // Join all positional arguments (Arg1, Arg2, ...) into a single message, so the
  // message doesn't need to be quoted on the command line.
  std::string message;
  for (int i = 1; args.contains("Arg" + std::to_string(i)); ++i) {
    if (!message.empty()) { message += ' '; }
    message += args.at("Arg" + std::to_string(i));
  }
  if (message.empty()) {
    std::cerr << "Usage: " << args.at("Arg0") << " <message text...>\n";
    return;
  }
  ...
  if (connection.open()) {
    ...
    // we disconnect. The call above is fire-and-forget - there is no response to wait for
    // if it succeeds.
    handler.handleFor(1s);

...
auto main() -> int { // NOLINT(bugprone-exception-escape)
	...
	handler.registerHandler<Messages::EventMsg>(Messages::event,
        [&connection](const Messages::EventMsg& msg) {
            if (msg.uGroupID == unknownGroup) {
                std::cout << std::format("Received event '{}' ({}): dwData = {}\n",
                    connection.eventName(msg.uEventID), msg.uEventID, msg.dwData);
            } else {
                std::cout << std::format("Received event '{}' ({}) in group {}: dwData = {}\n",
                    connection.eventName(msg.uEventID), msg.uEventID, msg.uGroupID, msg.dwData);
            }
